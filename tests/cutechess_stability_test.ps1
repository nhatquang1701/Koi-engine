...
if ([string]::IsNullOrWhiteSpace($StabilityScript)) {
    $StabilityScript = Join-Path (Split-Path -Parent $PSScriptRoot) 'tools\cutechess_stability.ps1'
}
$CampaignScript = Join-Path (Split-Path -Parent $PSScriptRoot) 'tools\cutechess_stability_campaign.ps1'
