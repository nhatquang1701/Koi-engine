...
struct SearchContext {
    ...
    int negamax(GameState& state, int depth, int alpha, int beta, int ply,
        ...
        if (null_move_candidate && null_move_is_safe(state, features)) {
                ...
                if (null_score >= beta) {
                    ...
                    if (depth >= 5) {
                        ...
                        verified_score = negamax(state, depth - 1, alpha, beta, ply,
                                                 verification_pv, previous_move, false);
                        if (aborted) {
                            return 0;
                        }
                    }
        ...
        for (const MoveMetadata& metadata : moves) {
            ...
            const bool lmr_base_candidate = move_number >= 4 && depth >= 4 && !checked &&
            ...
            if (lmr_candidate && before_quiet_features.has_value()) {
                const PositionFeatures after_quiet_features = state.position_features();
                const std::size_t enemy = before_quiet_features->side_to_move == Color::white ? 1U : 0U;
                king_zone_pressure = after_quiet_features.king_zone_attacks[enemy] >
                    before_quiet_features->king_zone_attacks[enemy];
                reducible_quiet = !state.in_check() &&
                    !quiet_move_is_forcing(*before_quiet_features, after_quiet_features, metadata);
            }
            ...
            const bool reduced = reducible_quiet && reduction > 0;
            const int child_depth = reduced ? full_child_depth - reduction : full_child_depth;
            if (reduced) {
        ...
        for (;;) {
            ...
            const auto search_root = [&context, &job](std::size_t move_index) {
                ...
                try {
                    ...
                    PrincipalVariation child_pv;
                    const int child_depth = job->depth - 1 +
                        (job->root_check_extension ? 1 : 0);
                    const int score = -context.negamax(child, child_depth,
...
struct SearchService::Impl {
    explicit Impl(std::shared_ptr<const Evaluator> evaluator, HashMemoryPolicy hash_memory_policy)
        : evaluator(std::move(evaluator)),
          table(std::make_shared<TranspositionTable>(512, std::move(hash_memory_policy))),
...
SearchHandle SearchService::start(GameState root, SearchLimits limits, SearchEventSink sink,
    ...
    state->worker = std::thread([state, evaluator, table, root = std::move(root), limits = std::move(limits),
        ...
        try {
            ...
            root.legal_moves_with_metadata(all_legal_moves);
