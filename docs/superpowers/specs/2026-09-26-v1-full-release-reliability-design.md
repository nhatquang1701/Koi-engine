...
Promote the published pre-release to a full 1.0.0 release after clearing every current search XFAIL/intermittent case and running broader reliability gates. The release now uses the bare `1.0.0` tag at `84dd8d57f62ba96315e6634be45148f5bd39fc29`, matching the user's newer GitHub naming rule; its current title is `First beta`. The old tag SHA, current release body, and four release assets are preserved locally before changing the tag again. Commit directly on the GitHub default `koi-engine` branch with detailed messages, create no new branch, and revert a pushed regression instead of resetting public history.
...
Finalize full-release wording before building clean-source packages and running exact-hash games, and keep the evidence locally. Only after all gates pass, promote the public release. The GitHub release body contains product/install information only; its assets are exactly Windows/Linux archives and matching `.sha256` files, with no completion-evidence files or verification claims. Temporarily draft the existing release; force-move its current bare `1.0.0` tag to the final default-branch commit, replace the four assets, set `prerelease=false`, title it `Koi Engine v1.0.0`, and publish. Read back title, tag target, flags, asset names, and downloaded hashes. If promotion fails, restore the saved tag, assets, body, and pre-release flags before making the release public again.
