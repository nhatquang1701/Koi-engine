...
- Cute Chess `-maxmoves` counts **full moves after the EPD opening**. A real one-game opening pilot with `-MaxMoves 1` ended at two plies. The Linux matrix uses 30 full moves for up to 60 plies at 1+0 and 6 full moves for up to 12 plies at 5+3; the validator uses the same units.
