...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    $OutputDirectory = Join-Path ([System.IO.Path]::GetTempPath()) (
        'koi-task5-release-verify-' + [guid]::NewGuid().ToString('N'))
}
$verificationRoot = [System.IO.Path]::GetFullPath($OutputDirectory)
$repositoryPrefix = $repositoryRoot.TrimEnd('\') + '\'
if ($verificationRoot -ieq $repositoryRoot -or $verificationRoot.StartsWith($repositoryPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "Verification output must be outside the repository: $verificationRoot"
}
...
try {
    ...
    $debugBuild = Join-Path $verificationRoot 'build-debug'
    $releaseBuild = Join-Path $verificationRoot 'build-release'
