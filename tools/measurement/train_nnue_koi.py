#!/usr/bin/env python3
"""Train a KOI-NNUE version 4 network for the ``halfka-king-bucket-v1`` feature set.

Reads either a ``koi-dataset-v1`` binary dataset (sparse indices, produced by
``koi_dataset.py``) or the ``FEN;cp;best_move`` text corpus, trains a
king-bucketed feature-transformer network on CPU with PyTorch, then quantizes
it so that the integer forward pass reproduces the C++ ``NnueWorker`` v4
semantics:
...
with ``b = min(7, (32 - pieces) / 4)``.  The exported container is version 4
(magic ``KOI-NNUE``, explicit ``hidden_shift``/``output_shift`` bytes) and the
metadata schema is ``koi-nnue-training-metadata-v2``.
