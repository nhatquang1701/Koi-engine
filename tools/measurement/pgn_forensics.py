...
DEFAULT_PGN_DIRECTORY = REPOSITORY_ROOT / "tests" / "data" / "games"
...
from tools.measurement import elo_oracle
...
def _require_output_path(path: Path) -> Path:
    ...
    if _is_inside(resolved, REPOSITORY_ROOT) and not _is_inside(resolved, ARTIFACT_ROOT):
        raise ForensicsError(
            f"generated reports inside the repository must be under artifacts: '{resolved}'"
        )
    return resolved
...
def _write_json(path: Path, report: Dict[str, Any]) -> None:
    output_path = _require_output_path(path)
    try:
        ...
        raise ForensicsError(f"unable to write report '{output_path}': {error}") from error
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        help="JSON report path (defaults to repository artifacts/verification)",
    )
    return parser
