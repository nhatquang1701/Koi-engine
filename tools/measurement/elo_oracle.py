...
def default_results_directory() -> Path:
    """Return the repository-local directory used for generated reports."""

    return REPOSITORY_ROOT / "artifacts" / "verification"


def default_report_path(stem: str = "elo-oracle") -> Path:
    """Return a unique repository-local report path by default."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        help="output JSON report path (default: repository artifacts/verification)",
    )
